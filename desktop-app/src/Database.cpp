...
bool Database::upsertDownload(const QVariantMap &r, QString *error) {
  ...
  const auto sensitive = protectJson({
    ...
    {QStringLiteral("tokenSigningKey"), r.value(QStringLiteral("tokenSigningKey"))},
    {QStringLiteral("tokenLifetimeSeconds"), r.value(QStringLiteral("tokenLifetimeSeconds"))},
    {QStringLiteral("tokenRefreshLeadSeconds"), r.value(QStringLiteral("tokenRefreshLeadSeconds"))},
